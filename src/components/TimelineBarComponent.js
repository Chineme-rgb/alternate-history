...
export default function TimelineBarComponent(props) {
    ...
    const markers = props.scenarioData.map((entry, index) =>
        <TimelineMarker key={index} index={index} label={`${entry.date} ${entry.time || ""}`} selected={props.activeEntry === index} updateActiveEntry={props.updateActiveEntry} addEntry={props.addEntry} themeDict={props.themeDict} />
    )
